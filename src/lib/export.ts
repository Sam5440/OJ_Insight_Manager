...
import type { DailyPoint, SubmissionItem } from '../types';
...
export function submissionsToCsv(rows: Array<SubmissionItem & { userName?: string }>): string {
  const lines = [
    ['序号', '用户', '平台', '提交ID', '题目ID', '题目名称', '题目链接', '提交时间(UTC+8)', '时间戳', '语言', '难度'].map(csvCell).join(','),
  ];
  rows.forEach((r, i) => {
    const dt = new Date((r.epoch_second + 8 * 3600) * 1000).toISOString().replace('T', ' ').slice(0, 19);
    lines.push(
      [
        i + 1,
        r.userName ?? '',
        PLATFORM_META[r.platform]?.name || r.platform,
        r.submission_id,
        r.problem_id,
        r.problem_name,
        r.problem_url,
...
export async function exportSubmissionsCsv(rows: SubmissionItem[], fileName: string) {
  const content = `\ufeff${submissionsToCsv(rows)}`;
  if (!isTauri) {
  ...
  const defaultPath = `${storage.exportDir}${slash}${fileName}`;
  const path = await save({ defaultPath, filters: [{ name: 'CSV', extensions: ['csv'] }] });
  if (!path) return;
  await invoke('write_export_file', { path, data: Array.from(new TextEncoder().encode(content)) });
}
...
export async function exportHeatmap(title: string, daily: DailyPoint[], startYear: number, endYear: number, format: 'png' | 'svg', startDay?:string, endDay?:string) {
  ...
  const defaultName = `OJ-Insight-${startDay&&endDay?'until-now':`${startYear}-${endYear}`}.${format}`;

  if (!isTauri) {
    if (format === 'svg') {
      downloadBlob(new Blob([svg], { type: 'image/svg+xml;charset=utf-8' }), defaultName);
      return;
    }
    const img = new Image();
  ...
  const separator = sep();
  const slash = storage.exportDir.endsWith('/') || storage.exportDir.endsWith('\\') ? '' : separator;
  const defaultPath = `${storage.exportDir}${slash}${defaultName}`;
